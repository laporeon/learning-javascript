...
*/
  ...
  var every = brasil.every((item) => item.length > 7);
  every
...
*/
  ...
  console.log(
    brasil.some((estado) => estado === "Ceará")
      ? "Ceará está incluído!"
      : "Ceará não fou incluído :("
  );
...
*/
  var map = newBrasil.map((item, index) => {
    return {
      id: item.id++,
      estado: (item.estado += " percente ao Brasil."),
    };
  });
...
*/
  var filter = map.filter((item, id) => item.id % 2 === 0);
