...
isTruthy(-1);
isTruthy({});
isTruthy({ id: 1, nome: "Curso JavaScript" });
isTruthy([]);
isTruthy(["Maçã", "Laranja"]);
isTruthy(function () {});
isTruthy([{}]);
...
carro.adicionarPessoas = function (pessoas) {
  ...
  if (assentosVagos === 0 && pessoas > 0) {
    return `O carro já está lotado!`;
  } else if (pessoas > assentosVagos) {
    return assentosVagos === 1
      ? `Só cabe mais 1 pessoa!`
      : `Só cabem mais ${assentosVagos} pessoas!`;
  } else {
...
carro.obterMarcaModelo(); // Este carro é um Fiat Argo
